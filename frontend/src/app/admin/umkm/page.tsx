...
export default function AdminUmkmPage() {
  ...
  const handleSubmit = async () => {
    if (!name.trim()) { toast("Nama UMKM wajib diisi!", "error"); return; }
    if (!ownerName.trim()) { toast("Nama Pemilik wajib diisi!", "error"); return; }
    if (whatsappError) { toast("Perbaiki kesalahan nomor WhatsApp terlebih dahulu!", "error"); return; }
    ...
    try {
      ...
      if (modalType === "Tambah") {
        const { error } = await adminCreateUmkm(payload);
        if (error) { toast(`Gagal menyimpan: ${error.message}`, "error"); return; }
        toast("Data UMKM berhasil ditambahkan! ✓");
      } else if (modalType === "Edit" && selectedId) {
        const { error } = await adminUpdateUmkm(selectedId, payload);
      ...
      toast("Terjadi kesalahan saat menyimpan data UMKM.", "error");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id: number, imageUrl?: string | null) => {
    if (!confirm("Apakah Anda yakin ingin menghapus data UMKM ini? Tindakan ini tidak bisa dibatalkan.")) return;
    try {
      const { error } = await adminDeleteUmkm(id);
      if (error) { toast(`Gagal menghapus: ${error.message}`, "error"); return; }
      if (imageUrl) await deleteImage(imageUrl);
      toast("Data UMKM berhasil dihapus.");
      fetchUmkm();
    } catch (err) {
      console.error(err);
      toast("Terjadi kesalahan saat menghapus data UMKM.", "error");
    }
  };
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        ...
        <button onClick={() => openModal("Tambah")} className="bg-[#1e3f20] hover:bg-[#152e17] text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2 whitespace-nowrap">
          + Tambah UMKM
        </button>
      </div>
            ...
            <tbody className="text-sm text-gray-600 align-middle">
              {isLoading ? (
                ...
                <tr><td colSpan={7} className="text-center py-8 text-gray-400">
                  {searchQuery ? "Tidak ada hasil yang cocok." : "Belum ada data UMKM."}
                </td></tr>
              ) : (
                  ...
                  <tr key={umkm.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <div className="w-12 h-12 bg-gray-200 rounded overflow-hidden">
                        {umkm.cover_image_url ? (
                          ...
                          <div className="w-full h-full bg-gray-300 flex items-center justify-center text-gray-400 text-xs font-bold">{umkm.name[0]}</div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => handleDelete(umkm.id, umkm.cover_image_url)} className="border border-red-100 text-red-500 hover:bg-red-50 px-2.5 py-1.5 rounded-lg transition-colors">
          ...
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-xl z-10 mx-4 max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 overflow-y-auto space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">WhatsApp (angka saja)</label>
                  <input type="text" value={whatsapp}
                    onChange={(e) => handleWhatsappChange(e.target.value)}
                    inputMode="numeric"
                    className={`w-full px-4 py-2.5 bg-white border rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-[#1e3f20] ${whatsappError ? "border-red-300 bg-red-50" : "border-gray-200"}`}
              ...
              <div>
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)}
                  ...
                  placeholder="Jelaskan produk atau jasa yang ditawarkan..."></textarea>
              </div>
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-3">
                  <label className="text-sm font-bold text-gray-700">Unggulan:</label>
                  <label className="relative inline-flex items-center cursor-pointer mt-1">
